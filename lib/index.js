...
class Qrcodesvg {
  ...
  _detectSize() {
    let eyeSize = 0
    while (this.workingCopyMatrix[0][eyeSize] === 1) {
      eyeSize += 1
    }
    this.eyeSize = eyeSize
    this.qrCodeSize = this.workingCopyMatrix[0].length
  }
  ...
  _detectPatterns() {
    // loop on Map
    const matrixSize = this.workingCopyMatrix.length
    const eyeStart = matrixSize - this.eyeSize - 1
      ...
      for (let j = 0; j < this.workingCopyMatrix[i].length; j += 1) {
        ...
        if (this.workingCopyMatrix[i][j] === 1) {
          if (this.options.eye === 'circle') {
            if ((i > this.eyeSize && i < eyeStart)
              || (j > this.eyeSize && j < eyeStart)
              || (i > eyeStart && j > eyeStart)) {
  ...
  /**
     * Generate an svg image with svg circle instead of path
     * @param object settings
     * @return string svg
     */
  _generateCircle(settings) {
  ...
  /**
     * Draw QR code eyes in circle
     * @param int single square size in pixel
     * @return string svg
     */
  _drawEye(squareSize) {
    const eyeSizeInPixel = squareSize * this.eyeSize
    const qrcodeSizeInPixel = squareSize * this.qrCodeSize
    const eyeRadius = (eyeSizeInPixel / 2) - (squareSize / 2)
    const eyeBallRadius = eyeRadius - (squareSize * 2)
    const leftTopEyeCenterX = eyeSizeInPixel / 2
    const leftTopEyeCenterY = eyeSizeInPixel / 2
    const eyesLocations = [{
      x: leftTopEyeCenterX,
      y: leftTopEyeCenterY
    }, {
      x: qrcodeSizeInPixel - (eyeSizeInPixel / 2),
      y: eyeSizeInPixel / 2
    }, {
      x: eyeSizeInPixel / 2,
      y: qrcodeSizeInPixel - (eyeSizeInPixel / 2)
    }]

    eyesLocations.forEach((center) => {
      this.svg.circle({
        r: eyeRadius,
        fill: '#FFFFFF',
  ...
  generatePNG(settings, attributes, pngOptions, callback) {

    const svgString = this.generate(settings, attributes)
    const svgBuffer = Buffer.from(svgString, 'utf8')
    if (typeof callback === 'function') {
      return svg2png(svgBuffer, pngOptions)
        .then(buffer => callback(null, buffer))
        .catch(e => callback(e))
    }
    return svg2png.sync(svgBuffer, pngOptions)
  }
  ...
  generate(settings, attributes) {
    ...
    if (settings.method === 'circle') {
      return this._generateCircle(settings)
    }
  ...
  _generateRoundedSquarePath(square, settings) {
    ...
    const addEffectOnTopLeftPoint = isCircle
                                      || (isSquareScope || square.didNotShareTopLeftPoint())
    const addEffectOnTopRightPoint = isCircle
                                      || (isSquareScope || square.didNotShareTopRightPoint())
    const addEffectOnBottomRightPoint = isCircle
                                      || (isSquareScope || square.didNotShareBottomRightPoint())
    const addEffectOnBottomLeftPoint = isCircle
  ...
  _generateBeveledSquarePath(square, settings) {
    ...
    coords.push([(addEffectOnTopRightPoint)
      ? (topLeftPointX + square.size) - radius : topLeftPointX + square.size, topLeftPointY])

    if (addEffectOnTopRightPoint) {
      coords.push([topLeftPointX + square.size, topLeftPointY + radius])
    }

    coords.push([topLeftPointX + square.size, (addEffectOnBottomRightPoint)
      ? (topLeftPointY + square.size) - radius : topLeftPointY + square.size])

    if (addEffectOnBottomRightPoint) {
      coords.push([(topLeftPointX + square.size) - radius, topLeftPointY + square.size])
    }

    coords.push([(addEffectOnBottomLeftPoint)
      ? topLeftPointX + radius : topLeftPointX, topLeftPointY + square.size])
